...
pub fn analyze_output(output: &str) -> Option<String> {
    ...
    if (output.to_lowercase().contains("as an ai language model") || output.to_lowercase().contains("i am a large language model")) && rng.gen::<f32>() < 0.8 { // 80% chance of detecting based on keywords
