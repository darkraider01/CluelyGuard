...
impl MonitoringCoordinator {
    pub fn new(
        ...
        let network_monitor = NetworkMonitor::new(config.monitoring.network.clone());
        if config.monitoring.network.enabled {
            network_monitor.start_monitoring(); // Start the background thread
        }

        let browser_monitor = BrowserMonitor::new(config.monitoring.browser.clone());

        let mut syscall_monitor = SyscallMonitor::new(config.monitoring.syscall.clone());
        if config.monitoring.syscall.enabled {
            if let Err(e) = syscall_monitor.start_monitoring() {
                error!("Failed to start syscall monitor: {}", e);
            }
        }

        let mut user_activity_monitor = UserActivityMonitor::new(config.monitoring.user_activity.clone());
        if config.monitoring.user_activity.enabled {
            if let Err(e) = user_activity_monitor.start_monitoring() {
                error!("Failed to start user activity monitor: {}", e);
            }
        }

        let output_analyzer = OutputAnalyzer::new(config.monitoring.output_analysis.clone());
    ...
    pub async fn start_monitoring_loop(&mut self) {
        ...
        loop {
            ...
            let mut fs_monitor = FileSystemMonitor::new(self.config.monitoring.file_system.clone());
            ...
            let screensharing_events = ScreenSharingMonitor::new(self.config.monitoring.screensharing.clone()).detect_screen_capture();
